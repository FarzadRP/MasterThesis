...
lgraph = modifiedSSDlayers(inputSize, numClasses, 'resnet50');
...
[SSDdetector, dayInfo] = trainSSDObjectDetector(trainData, lgraph, options); save('SSDdetector', 'SSDdetector')

%% Test
ap_Detector_dayData = iDetectEvaluate(SSDdetector, pp_day_testData);
ap_Detector_nightData = iDetectEvaluate(SSDdetector, pp_night_testData);
ap_Detector_rainyData = iDetectEvaluate(SSDdetector, pp_rainy_testData);
ap_Detector_completeData = iDetectEvaluate(SSDdetector, pp_complete_testData);

%% Save results
Results_SSDdetector = table(ap_Detector_dayData, ap_Detector_nightData, ap_Detector_rainyData, ap_Detector_completeData);
save('Results_SSDdetector', 'Results_SSDdetector')
